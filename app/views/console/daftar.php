...
<html>
	...
	<body class="bodyy" style="background-color: aliceblue; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; font-family: Arial, sans-serif; flex-direction: column; padding: 20px; box-sizing: border-box;">
		...
		<div class="accwrapper">
			<div class="accinner">
				<form action="<?= BASEURL; ?>/console/addAkun" method="post">
					...
					<div class="accform-wrapper">
						<select name="roleRank" id="roleRank" class="accform-control" required>
							<option value="" disabled <?= $data['selectedRole'] == null ? 'selected' : ''?> hidden>-- Select an option --</option>
							<option value="1" <?= $data['selectedRole'] == 1 ? 'selected' : ''?>>Turut mengelola</option>
							<option value="2" <?= $data['selectedRole'] == 2 ? 'selected' : ''?>>Turut Mengundang</option>
						</select>
					</div>
